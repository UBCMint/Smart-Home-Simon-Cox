const buttons = [
    "#controls__channel_up",
    "#controls__channel_down",
    "#controls__power_btn",
];
const timeout = 2000;
...
function printHistory() {
    let num = 1;
    for (i = history_ptr; i < HISTORY_SIZE; i++) {
        if (history[i] != undefined) {
            //console.log(history_ptr + "  " + "action " + num + " is " + history[i]);
            num++;
        }
    }
    for (i = 0; i < history_ptr; i++) {
        if (history[i] != undefined) {
            //console.log(history_ptr + "  " + "action " + num + " is " + history[i]);
            num++;
        }
...
const generateGraph = () => {
    $.get("/api/model_output", function(data) {
        ...
        if (data.result === 1) {
            // console.log("history size:   " + history_ptr);
            history[history_ptr] = currentBtn;
